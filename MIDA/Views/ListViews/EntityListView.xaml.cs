...
public partial class EntityListView : UserControl
{
    ...
    private async Task CleanPackageList()
    {
        ...
        TigerHash tagClass = new(Helpers.GetClassHashForStrategy(typeof(S80808673), Strategy.CurrentStrategy));
    ...
    private void RefreshEntityList()
    {
        ...
        if (items.Count > 0)
            EntityList.ScrollIntoView(items[0]);
        ...
        else
        {
            ...
            await Task.Run(() =>
            {
                Parallel.ForEach(vals, parallelOptions, val =>
                {
                    ...
                    {
                        if (dataEntry.DataResource.GetValue(entry.GetReader()) is S839D8080 name && name.EntityName.IsValid())
                        {
            ...
            await Task.Run(() =>
            {
                Parallel.ForEach(resources, parallelOptions, val =>
                {
                    ...
                    var sb546 = (S80805B83)resource.TagData.Unk18.GetValue(resource.GetReader());
                    foreach (var entry in sb546.Unk80)
                    {
