...
public partial class AudioListView : UserControl
{
    ...
    private async Task MakeSoundbanks()
    {
        ...
        PackageList.UseVirtualizingStackPanel = true;
        ...
        await Task.Run(() =>
        {
            ...
            Parallel.ForEach(hashes, hash =>
            {
                ...
                {
                    string name = bank.TagData.SoundbankName.GetString();
                    //if (bank.TagData.GetSoundbank() is not null)
                    //{
                    //    name = bank.TagData.GetSoundbank().TagData.SoundBank.GetNameFromStringHash();
                    //}

                    PackageList.PackageItems.Add(new PackageItem
                    {
                        Name = name,
                        Subname = string.Join('_', PackageResourcer.Get().GetPackage(bank.Hash.PackageId).GetPackageMetadata().Name.Split('_').Skip(1).SkipLast(1)),
    ...
    private void RefreshSoundList()
    {
        ...
        Dispatcher.Invoke(() =>
        {
            ...
            {
                5 => items.OrderByDescending(x => x.Hash.Hash32).ToList(),
            ...
            if (items.Count > 0)
                AudioList.ScrollIntoView(items[0]);
