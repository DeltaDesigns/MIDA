...
public class SchemaDeserializer : Strategy.StrategistSingleton<SchemaDeserializer>
{
    ...
    private FieldInfo[] GetStrategyFields(FieldInfo[] getFields)
    {
        ...
        var fields = getFields.Where(f => !f.GetCustomAttributes<SchemaFieldAttribute>().Any() ||
       (_strategy >= GetAttribute<SchemaFieldAttribute>(f).Strategy && !GetAttribute<SchemaFieldAttribute>(f).Obsolete)).ToArray();
