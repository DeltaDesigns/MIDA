...
public class EntityModelParent : EntityComponent
{
    ...
    public S80808678 Reader => ((S80808678)TagData.Unk18.GetValue(GetReader()));
    ...
    {
        if (Reader.Unk408.Count == 0 || Reader.Unk418.Count == 0)
            return null;
        ...
        for (int i = 0; i < Reader.Unk418.Count; i++)
        {
            var u = Reader.Unk418[GetReader(), i];
            if (u.Unk02 < 0)
                continue;
            ...
            if (end >= Reader.Unk408.Count)
                continue;

            for (int j = start; j < end; j++)
            {
                var m = Reader.Unk38[Reader.Unk408[GetReader(), j].Value];
